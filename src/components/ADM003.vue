  ...
  <div class="contents-wrap">
    ...
    >
      <template v-slot:item.data02="{ item }">
        <router-link to="/PCMS/M01/ADM004">{{ item.data02 }}</router-link>
      </template>
      <template v-slot:item.data06="{ item }">
        ...
        <div v-if="item.state === 3">
          <div class="table-inner-btn--text">문서등록완료</div>
          <div class="table-inner-btn--text">검수진행중</div>
        </div>
        ...
        <div v-if="item.state === 6">
          <div class="table-inner-btn--text">문서재등록완료</div>
          <div class="table-inner-btn--text">재검수진행중</div>
        </div>
        <div v-if="item.state === 7">
          <div class="table-inner-btn--text text-color--secondary font-weight-medium">문서등록완료</div>
          <div class="table-inner-btn--text text-color--green font-weight-medium">검수완료</div>
        </div>
        <div v-if="item.state === 8">
          <div class="table-inner-btn--text text-color--secondary font-weight-medium">문서재등록완료</div>
          <div class="table-inner-btn--text text-color--green font-weight-medium">재검수완료</div>
        </div>
      </template>
    </v-data-table>
    </div>
        ...
        <v-card class="default-dialog">
          ...
          <v-card-text>
            <ul class="inspector-list">
              <li v-for="item in inspectorList" :key="item.items"
                class="inspector-list__item"
              >
                <dl class="item">
                  <dt class="item__title">부서</dt>
                  <dd class="item__contents">
                    <span class="depart">{{ item.department00 }}</span>
                    <span class="depart" v-if="item.department01.length > 0">{{ item.department01 }}</span>
                  </dd>
                </dl>
                <dl class="item">
                  <dt class="item__title">검수자</dt>
                  <dd class="item__contents">{{ item.name }}</dd>
                </dl>
              </li>
            </ul>
          </v-card-text>
...
export default {
  ...
  data: () => ({
    dialog01: false, // dialog show
    dialog02: false, // dialog show
    modelBtnDisabled: true, // 문서다운로드버튼 disable, false 시 클릭 가능
    pageTitle: {
      pageTitleText: "상품등록 및 결재요청",
      infoPathList: [
        {text: "홈"},
        {text: "상품관리"},
        {text: "상품등록 및 결재요청"}
